        ...
        <div>
            ...
            <div key={index} className='mb-8 flex flex-wrap  text-left lg:justify-center '>
                ...
                <h6 className='mb-2 font-semibold'>
                    {experience.cousre} - <span className='text-sm text-indigo-200'>{experience.College}</span>
                </h6>
                <p className='mb-4 '>{experience.description}</p>
                {experience.Subjects.map((tech, index) => (
                    <span key={index} className='mr-2 mt-4 rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-indigo-400'>{tech}</span>
                ))}
